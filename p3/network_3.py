...
class NetworkPacket:
    ...
    HEADER_LEN = FRAGMENT_FLAG_LEN + FRAGMENT_OFFSET_LEN + FRAGMENT_ID_LEN + \
        HEADER_DST_LEN + HEADER_SRC_LEN + HEADER_LENGTH_LEN + HEADER_CHECKSUM_LEN
    ...
    def to_byte_S(self):
        ...
        buffer.write(str(self.identifier).zfill(self.FRAGMENT_ID_LEN))
    ...
    def from_byte_S(self, byte_S):
        ...
        fragment_id = int(header_buffer.read(self.FRAGMENT_ID_LEN))
        src_addr = int(header_buffer.read(self.HEADER_SRC_LEN))
        dst_addr = int(header_buffer.read(self.HEADER_DST_LEN))

        return NetworkPacket(fragment_flag, fragment_offset, fragment_id, dst_addr, src_addr, data_S=data)
...
class PacketFragmenter:
    ...
    def fragment(cls, src_addr, dst_addr, data_s, mtu_size):
        ...
        for i in range(num_packets):
            ...
            packets.append(NetworkPacket(Flags.MORE_FRAGMENTS, offset, identifier, dst_addr, src_addr, fragment_contents))
            offset += packet_size
    ...
    def fragment_packet(cls, packet: NetworkPacket, mtu_size):
        ...
        for i in range(num_packets):
            fragment_contents = data_s[:packet_size]
            data_s = data_s[packet_size:]
            packets.append(NetworkPacket(Flags.MORE_FRAGMENTS, offset, packet.identifier, packet.dest_addr, packet.src_addr, fragment_contents))
            offset += packet_size
...
class Host:
    ...
    def udt_send(self, dst_addr, data_S):
        packets = PacketFragmenter.fragment(self.addr, dst_addr, data_S, self.out_intf_L[0].mtu)
    ...
    def reassemble(self, identifer):
        buffer = StringIO()
        offset = 0
        sorted_frames = sorted(self.frames[identifer], key=lambda packet: packet.fragment_offset)
    ...
    def add_fragment_to_frame(self, packet: NetworkPacket):
        if packet.identifier not in self.frames:
            self.frames[packet.identifier] = []
        self.frames[packet.identifier].append(packet)

        if packet.fragment_flag == Flags.LAST_FRAGMENT:
            contents = self.reassemble(packet.identifier)
    ...
    def udt_receive(self):
        ...
        if pkt_S is not None:
            ...
            self.add_fragment_to_frame(pkt)
...
class Router:
    ...
    # @param routes: A dictionary routes that specifies the interface
    # to forward outgoing packets onto
    def __init__(self, name, intf_count, max_queue_size, routes):
        self.stop = False #for thread termination
        self.name = name
        ...
        for i in range(len(self.in_intf_L)):
            ...
            try:
                ...
                if pkt_S is not None:
                    ...
                    src_addr = p.src_addr
                    dst_addr = p.dest_addr
                    src_table = self.routes[src_addr]
                    if not dst_addr in src_table:
                        print("no route configured to forward from src=%d, dest=%d" % (src_addr, dst_addr))
                        return
                    else:
                        interface = src_table[dst_addr]
                        print("resolved interface %d for src=%d, dest=%d" % (interface, src_addr, dst_addr))
                        for fragment in fragments:
                            self.out_intf_L[interface].put(fragment.to_byte_S(), True)
                            print('%s: forwarding packet "%s" from interface %d to %d with mtu %d' \
                                % (self, fragment, i, interface, self.out_intf_L[interface].mtu))
